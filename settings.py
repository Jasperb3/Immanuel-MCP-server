...
class ServerSettings(BaseSettings):
    ...
    ephemeris_path: Path = Field(
        default=Path.home() / ".immanuel" / "ephemeris",
        env="IMMANUEL_EPHEMERIS_PATH"
    )
    auto_download_ephemeris: bool = Field(
        default=False,
        env="IMMANUEL_AUTO_DOWNLOAD_EPHEMERIS"
    )
    ...
    default_points: List[str] = Field(
        default=["ASC", "MC", "NORTH_NODE", "SOUTH_NODE"],
        env="IMMANUEL_DEFAULT_POINTS"
    )
    default_asteroids: List[str] = Field(
        default=[],
        env="IMMANUEL_DEFAULT_ASTEROIDS"
    )
...
class InterpretationSettings(BaseSettings):
    ...
    house_keywords: Dict[int, Dict[str, str]] = Field(
        default={
            ...
            12: {"keyword": "Transcendence", "domains": "Spirituality, hidden things, isolation, service"}
        }
    )
