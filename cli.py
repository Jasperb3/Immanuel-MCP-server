...
class ImmanuelCLI:
    ...
    def format_chart_output(self, chart_data: Dict[str, Any]) -> str:
        ...
        for obj_name, obj_data in objects.items():
            ...
            output.append(
                f"{obj_name:<12} {sign:<12} {degree:<15} {house:<8} {retro:<6}"
            )
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Immanuel MCP CLI - Calculate astrological charts"
    )
    ...
    example_parser = subparsers.add_parser(
        "example",
        help="Run example chart calculation"
    )
    
    # Calculate command
    calc_parser = subparsers.add_parser(
        "calculate",
        help="Calculate a natal chart"
    )
    calc_parser.add_argument(
        "--datetime",
        required=True,
        help="Birth datetime in ISO format (e.g., 2000-01-01T10:00:00)"
    )
    calc_parser.add_argument(
        "--latitude",
        required=True,
        help="Latitude (e.g., 34.05N or -34.05)"
    )
    calc_parser.add_argument(
        "--longitude",
        required=True,
        help="Longitude (e.g., 118.24W or -118.24)"
    )
    calc_parser.add_argument(
        "--timezone",
        help="Timezone (e.g., America/Los_Angeles)"
    )
    calc_parser.add_argument(
        "--house-system",
        default="placidus",
