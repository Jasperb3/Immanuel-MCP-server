...
class ChartSettings(BaseModel):
    ...
    include_objects: List[str] = Field(
        default_factory=list, description="Additional objects to include"
    )
...
class ChartRequest(BaseModel):
    ...
    settings: Optional[ChartSettings] = Field(
        default_factory=ChartSettings, description="Chart settings"
    )
...
class ChartResponse(BaseModel):
    ...
    aspects: List[Dict[str, Any]] = Field(
        default_factory=list, description="Aspects between objects"
    )
    ...
    class Config:
        schema_extra = {
            "example": {
                ...
                "houses": {
                    "house_1": {"number": 1, "sign": "Aries", "degree": 15.0, "objects": ["Mars"]}
                },
            }
        }
...
class TransitRequest(BaseModel):
    ...
    aspect_orbs: Optional[Dict[str, float]] = Field(
        None, description="Custom orbs for transit aspects"
    )
    include_planets: Optional[List[str]] = Field(
        None, description="Which planets to include in transits"
    )
