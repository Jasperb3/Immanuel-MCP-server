...
class ChartService:
    ...
    async def calculate_single_chart(
        ...
        # Create the subject
        native = immanuel.charts.Subject(
            date_time=dt,
            latitude=lat,
            longitude=lon,
            timezone=tz
    ...
    async def calculate_batch_charts(
        ...
        tasks = [
            self.calculate_single_chart(subject, chart_type, settings)
    ...
    def _prepare_config(self, settings: ChartSettings) -> Dict[str, Any]:
        ...
        config = {
            'house_system': getattr(chart_const, settings.house_system.upper(), chart_const.PLACIDUS)
        }
        ...
        if hasattr(chart, 'houses'):
            for house in chart.houses.values():
                houses[f"house_{house.number}"] = {
                    ...
                    'objects': [obj.name for obj in house.objects] if hasattr(house, 'objects') else []
                }
    ...
    def _interpret_dignities(self, obj_name: str, dignities: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    async def _calculate_synastry(self, chart1: Dict[str, Any], chart2: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _calculate_composite(self, chart1: Dict[str, Any], chart2: Dict[str, Any]) -> Dict[str, Any]:
